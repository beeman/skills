...
10. Write or refresh a commit message that matches the repository history, includes a body, and describes only the tracked staged diff. Do not mention thread discussion, related ideas, or requested changes that are not present in the committed files. When the local style is unclear, use a conventional commit.
